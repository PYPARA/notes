...
注意，要选择节点(使用 range1)，必须先确定给定节点(p1)在其父节点 `childNodes` 集合中 的索引。而要选择节点的内容(使用 range2)，则不需要这样计算，因为可以直接给 `setStart()` 和 `setEnd()` 传默认值。虽然可以模拟 `selectNode()` 和 `selectNodeContents()`，但 `setStart()` 和 `setEnd()` 真正的威力还是选择节点中的某个部分。
